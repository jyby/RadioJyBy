...
** Tag Line
   ...
   Putting back the human connection in the act of listening to music.
...
** Conditional to funding
   ...
   - Identification independent of phone number :: 
     - Base User Authentification on pair of RSA keys
   ...
   - Decentralized Mode ::
     - Rather than keeping the message on a central sever (e.g. Whatsapp, Telegram), transmit the audio message only if the correspondent is connected (e.g. Skype mode)
